...
var TerraFrontendStorageService = /** @class */ (function (_super) {
    ...
    function TerraFrontendStorageService(_terraLoadingSpinnerService, _http, _translation) {
        var _this = _super.call(this, _terraLoadingSpinnerService, _http, '/rest/storage/frontend/file') || this;
        _this.isImagePreviewEnabled = true;
        _this._storageInitialized = false;
        _this._storageListSubject = new BehaviorSubject_1.BehaviorSubject(null);
        _this.queue = new terra_upload_queue_1.TerraUploadQueue('/rest/storage/frontend/file');
        _this._metadataCache = {};
        _this.name = _translation.translate('terraFileBrowser.myFiles');
        return _this;
    }
    ...
    TerraFrontendStorageService.prototype.createDirectory = function (path) {
        ...
        request.subscribe(function () {
            _this._storageListSubject.next(_this._storageList.insertObject(s3_storage_object_interface_1.createS3StorageObject(path)));
        });
        return request;
    };
    ...
    TerraFrontendStorageService.prototype.uploadFile = function (file, path) {
        ...
        item.beforeUpload(function () {
            _this._storageListSubject.next(_this._storageList.insertObject(s3_storage_object_interface_1.createS3StorageObject(item.pathname)));
        });
        item.onSuccess(function (response) {
            var s3Data = JSON.parse(response);
            _this._storageListSubject.next(_this._storageList.insertObject({
                eTag: s3Data.eTag,
                key: s3Data.key,
        ...
        item.onError(function () {
            ...
            _this._storageListSubject.next(_this._storageList);
        });
    ...
    TerraFrontendStorageService.prototype.getMetadata = function (key) {
        ...
        if (this._metadataCache.hasOwnProperty(key)) {
            return Observable_1.Observable.from([this._metadataCache[key]]);
        }
        this.setAuthorization();
        ...
        request.subscribe(function (metadata) {
            _this._metadataCache[key] = metadata;
        }, function () {
            delete _this._metadataCache[key];
        });
        return request;
    };
    ...
    TerraFrontendStorageService.prototype.deleteFiles = function (keyList) {
        ...
        request.subscribe(function () {
            ...
            _this._storageInitialized = false;
            _this._storageListSubject.next(null);
        });
        return request;
    };
    TerraFrontendStorageService.prototype.initStorageList = function (continuationToken) {
        ...
        this.mapRequest(this.http.get(url, { headers: this.headers })).subscribe(function (results) {
            var storageList = _this._storageListSubject.getValue() || new terra_storage_object_list_1.TerraStorageObjectList();
            storageList.insertObjects(results.objects);
            _this._storageListSubject.next(storageList);
    ...
    TerraFrontendStorageService = __decorate([
        core_1.Injectable(),
        __metadata("design:paramtypes", [_1.TerraLoadingSpinnerService, http_1.Http, angular_l10n_1.TranslationService])
    ], TerraFrontendStorageService);
    return TerraFrontendStorageService;
}(terra_base_storage_interface_1.TerraBaseMetadataStorageService));
