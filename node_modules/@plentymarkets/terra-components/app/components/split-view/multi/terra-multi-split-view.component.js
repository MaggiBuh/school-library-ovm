...
var terra_multi_split_view_config_1 = require("./data/terra-multi-split-view.config");
var router_1 = require("@angular/router");
var nextSplitViewId = 0;
var TerraMultiSplitViewComponent = /** @class */ (function () {
    function TerraMultiSplitViewComponent(zone, _router) {
        this.zone = zone;
    ...
    TerraMultiSplitViewComponent.prototype.ngOnInit = function () {
        ...
        if (!util_1.isNullOrUndefined(this._router) && !util_1.isNullOrUndefined(this.inputComponentRoute)) {
            // check if the given route exists in the route config
            if (this.routeExists(this.inputComponentRoute)) {
                // register event listener
                this._router.events
                    .filter(function (event) { return event instanceof router_1.NavigationStart && event.url === _this.inputComponentRoute; })
                    .subscribe(function (path) {
                    _this.updateViewport(_this.inputConfig.currentSelectedView, true);
                });
            }
        }
        this.inputConfig.addViewEventEmitter.subscribe(function (value) {
            // synchronize modules array with input config
            _this.addToModulesIfNotExist(value);
            // set the selected view
            _this.setSelectedView(value);
        });
        this.inputConfig.deleteViewEventEmitter.subscribe(function (value) {
            // update modules array
            var viewToSelect = _this.removeFromModules(value);
            // select the parent view
            _this.setSelectedView(viewToSelect);
        });
        this.inputConfig.resizeViewEventEmitter.subscribe(function (value) {
            _this.resizeViewAndModule(value);
        });
        this.inputConfig.setSelectedViewEventEmitter.subscribe(function (value) {
            _this.setSelectedView(value);
        });
    };
    ...
    TerraMultiSplitViewComponent.prototype.setSelectedView = function (view) {
        ...
        if (inputModule !== this.getModuleOfView(view)
            && !util_1.isNullOrUndefined(inputModule)) {
    ...
    TerraMultiSplitViewComponent.prototype.routeExists = function (route) {
        ...
        while (routeLevel < path.length) {
            ...
            var foundRoute = routes.find(function (route) { return route.path === path[routeLevel]; });
            if (foundRoute) {
                // into deep
                routeLevel++;
